...
namespace AESES_Databases
{
    internal class BescomStates
    {
        ...
        public static void SetStates(STATES StatesDB, BescomRecord acsRecord)
        {
            ...
            foreach (DataRow stateRow in BescomParser.StateRec_Tbl.Rows)
            {
                ...
                if (!_uniqueStates.ContainsKey(tempNamePair))
                {
                    ...
                    statesObj.SetValue("names_0", stpFileDefault.ACSRecords[name0Value].InfoDict["message"]);
                    statesObj.SetValue("names_1", stpFileDefault.ACSRecords[name1Value].InfoDict["message"]);
            ...
            foreach (DataRow stateRow in BescomParser.StateRec_Tbl123.Rows)
            {
                ...
                if (name2Value != 13)
                {
                    if (!_uniqueStates.ContainsKey(tempNamePair))
                    {
                        ...
                        statesObj.CurrentRecordNo = ++statesRec;
                        statesObj.SetValue("names_0", stpFileCustom.ACSRecords[name2Value].InfoDict["message"]);
                        statesObj.SetValue("names_1", stpFileCustom.ACSRecords[name3Value].InfoDict["message"]);
                        statesObj.SetValue("description", tempNamePair);
                        _uniqueStates.Add(tempNamePair, statesRec);
                    }
                }
                ...
                if (name2Value == 13)
                {
                    ...
                    if (!_uniqueStates.ContainsKey(tempNamePair))
                    {
                        ...
                        statesObj.SetValue("names_0", stpFileCustom.ACSRecords[name2Value].InfoDict["message"]);
                        statesObj.SetValue("names_1", stpFileCustom.ACSRecords[name3Value].InfoDict["message"]);
                        statesObj.SetValue("names_2", stpFileCustom.ACSRecords[name3Value].InfoDict["message"]);
                        statesObj.SetValue("names_3", stpFileCustom.ACSRecords[name3Value].InfoDict["message"]);
                        statesObj.SetValue("description", tempNamePair);
                        _uniqueStates.Add(tempNamePair, statesRec);
                    }
        ...
        public static int SetStates2(BescomRecord acsRecord)
        {
            ...
            {
                tempNamePair = $"{stpFileDefault.ACSRecords[name0Value1-200].InfoDict["message"].Trim()}/{stpFileDefault.ACSRecords[name1Value2-200].InfoDict["message"].Trim()}";
                pState = _uniqueStates[tempNamePair];
                if(pState==327)
                {
                    pState = 317;
