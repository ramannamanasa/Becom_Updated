...
namespace AESES_Databases
{
    class BescomConverter
    {
        ...
        static readonly string[] XrefFieldsSUB = { "Category","filename", "Type", "OSI Station", "SCADA Key", "Object", "OSI AOR", "Point Address", "Calculated", "OSI Record", "pRTU", "Feptype", "StsrRecord", "CtlRecord", "AESRec", "SCADA Name", "station", "pState" };
        static GenericTable XrefSUB = new GenericTable(XrefFieldsSUB, "OSI Record");
        ...
        public BescomConverter()
        {
           ...
           //BescomFep.ConvertChannelGroup(FepDB);
            ...
            BescomFep.ConvertRTUNewControl(FepDB, Xref);
            BescomFep rtuDefn = new BescomFep(FepDB, Xref);
            BescomFep.ConvertRtuDefn(FepDB, Xref);
            BescomFep.GlobalScanDefn(FepDB);    
            BescomICCP.ConvertControlCenterInfo(ICCPDB);
            BescomICCP.ImportDataSet(ICCPDB);
            BescomICCP.ImportPoints(ICCPDB, XrefSUB);
            #region FEPCHECK
            //FEPCHECK
            try
            { 
           ...
           // ScadaDB.PopulateAndScadaValidate();
            ...
            try
            {
                ICCPDB.PopulateAndIccpBuild(false);
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message);
        ...
        public void ConvertAorGroups(FEP FepDb)
        {
        ...
        public void ConvertStations()
        {
            ...
            foreach (DataRow stationRow in BescomParser.mastercfg_filteredTbl.Rows)
            {
                ...
                string stationKey = stationRow["SIETID"].ToString();
        ...
        public void ConvertSCADA()
        {
            ...
            DbObject setpointObj = ScadaDB.GetDbObject("SETPOINT");
            int statusRec = 0;
            int setpointrec = 0;
            ...
            foreach (BescomFile stsrFile in BescomParser.stsrSet.Files.Values)
            {
                ...
                foreach (BescomRecord stsrRecord in stsrFile.ACSRecords)
                {
                    ...
                    if (BescomParser.ctlSet.Files.Keys.Contains(station))
                    {
                        foreach (BescomRecord ctlRecord in BescomParser.ctlSet.Files[station].ACSRecords)
                        {
                                ...
                                //&&ctlRecord.InfoDict["solicited_point"].Equals(stsrRecord.Record.ToString()))
                            {
                                ...
                                if(PrevName == ctlRecord.InfoDict["description"]|| name== "SF6 GAS PRESSURE")
                                {
                                    continue;
                                }
                    ...
                    if(pState==406)
                    {
                        statusObj.SetValue("ConfigBits", pState);
                        statusObj.SetValue("StateCalc", 5);
                    }
                    if (pState == 327||name.StartsWith("SPARE"))
                    {
                        pState = 205;
                        statusObj.SetValue("pStates", pState);
                        //statusObj.SetValue("ConfigBits", pState);
                    }
                    if(name== "SF6 GAS PRESSURE")
                    {
                        pState = 227;
                        statusObj.SetValue("pStates", pState);
                    }
                    if (name == "RELAY GROUP 1" || name == "RELAY GROUP 2" || name == "RELAY GROUP 3")
                    {
                        pState = 427;
                        statusObj.SetValue("pStates", pState);
                    }
                    if (stsrRecord.InfoDict["abnormal_list"].Equals("1") && stsrRecord.InfoDict["abnormal_definition"].Equals("A")) 
                    {
            ...
            foreach (BescomFile ctlFile in BescomParser.ctlSet.Files.Values)
            {
                ...
                foreach (BescomRecord ctlRecord in ctlFile.ACSRecords)
                {
                    ...
                    //|| name.Contains("SPARE 1")|| name.Contains("SPARE 2")|| name.Contains("SPARE 3")|| name.Contains("SPARE 4")|| name.Contains("SPARE 5"))
                    if (name.StartsWith("FPI") || name.Contains("ACTIVATE REVERSE FLOW") )
                    {
                        statusObj.CurrentRecordNo = statusRec++;
                        statusObj.SetValue("Name", name);
                        ...
                        int feptype = 3;
                        object[] xrefObjects = { filename, type, statusObj.GetValue("pStation", 0), statusObj.GetValue("Key", 0), "STATUS", statusObj.GetValue("pAORGroup", 0), ctlRecord.Record.ToString(), "R", statusObj.CurrentRecordNo.ToString(), pRTU, feptype,"", ctlRecord.Record.ToString(), "", statusObj.GetValue("Name", 0) , value, pState }; //statusObj.GetValue("Name", 0) FEP RTU_CONTROL name (from ctl files) statusObj.GetValue("pRTU", 0)
                                                                                                                                                                                                                                                                                                                                 //{ "filename", "Type", "OSI Station", "SCADA Key", "Object", "OSI AOR", "Point Address", "Calculated", "OSI Record" };//STSR
                        Xref.AddRecordSetValues(xrefObjects);
                ...
                foreach (BescomRecord ctlRecord in ctlFile.ACSRecords)
                {
                    DataRow setpointRow = BescomParser.mastercfg_filteredTbl.GetInfo(new object[] { filenum });
                    string name = ctlRecord.InfoDict["description"];
                    if(name.Contains("GROUP A/B ACTIVE")||name.Contains("FORWARD RELAY GROUP")||name.Contains("REVERSE RELAY GROUP")||name.Contains("SPARE SETPOINT"))
                    {
                        setpointrec++;
                        setpointObj.CurrentRecordNo = setpointrec;
                        setpointObj.SetValue("Name", name);
                        setpointObj.SetValue("Type", 1);
                        setpointObj.SetValue("pAORGroup", SetAor2(ctlRecord, filenum));
                        setpointObj.SetValue("pStation", SetpStation2(ctlRecord, filenum));
                        setpointObj.SetValue("pALARM_GROUP", 2);
                        setpointObj.SetValue("HiLimit", 1);
                        int pRTU = GetpRTU(ctlRecord, filenum);
                        SetArchiveGroup(setpointObj);
                        setpointObj.SetValue("pRTU", pRTU);
                        if (filenum != 0)
                            try
                            {
                                setpointObj.SetValue("Key", ScadaDB.GetNextKey(9, pRTU));
                            }
                            catch (Exception ex)
                            {
                                Console.WriteLine(ex.Message);
                       ...
                       // int pState = 223;
                        ...
                        string filename = ctlFile.Name;
                        int feptype = 3;
                        object[] xrefObjects = { filename, type, setpointObj.GetValue("pStation", 0), setpointObj.GetValue("Key", 0), "SETPOINT", setpointObj.GetValue("pAORGroup", 0), ctlRecord.Record.ToString(), "R", setpointObj.CurrentRecordNo.ToString(), pRTU, feptype, "", ctlRecord.Record.ToString(), "", setpointObj.GetValue("Name", 0), value, "NA" }; //statusObj.GetValue("Name", 0) FEP RTU_CONTROL name (from ctl files) statusObj.GetValue("pRTU", 0)
                                                                                                                                                                                                                                                                                                                                   //{ "filename", "Type", "OSI Station", "SCADA Key", "Object", "OSI AOR", "Point Address", "Calculated", "OSI Record" };//STSR
                        Xref.AddRecordSetValues(xrefObjects);
                    }
           ...
           foreach (BescomFile stsdFile in BescomParser.stsdSet.Files.Values)
            ...
            foreach (BescomFile sstsrFile in BescomParser.sstsr.Files.Values)
            {
                ...
                foreach (BescomRecord sstsrRecord in sstsrFile.ACSRecords)
                {
                   ...
                   // int pRTU = GetpRTU(sstsrRecord, stationid);
                    ...
                    statusObj.SetValue("pStation", SetpStationSubstation(sstsrRecord, stationid));
                    ...
                    string disp_num = sstsrRecord.InfoDict["display_number"];
                    ...
                    object[] xrefObjects = {"R", filename, type, disp_num, statusObj.GetValue("Key", 0), "STATUS", statusObj.GetValue("pAORGroup", 0), sstsrRecord.Record.ToString(), "R", statusObj.CurrentRecordNo.ToString(), 0, feptype, StsrRecNo, CtlRecNo, "", statusObj.GetValue("Name", 0), value }; //statusObj.GetValue("Name", 0) FEP RTU_CONTROL name (from ctl files) statusObj.GetValue("pRTU", 0)

                    XrefSUB.AddRecordSetValues(xrefObjects);
                }
            }
            #endregion
            ...
            foreach (BescomFile stlmrFile in BescomParser.stlmr.Files.Values)
            {
                ...
                foreach (BescomRecord stlmrRecord in stlmrFile.ACSRecords)
                {
                    ...
                    string value = string.Empty;
                    _StationDict.TryGetValue(pst, out value);
                    string disp_num = stlmrRecord.InfoDict["display_number"];
                    ...
                    object[] xrefObjects = { "R",stlmrFile.Name, type, disp_num, analogObj.GetValue("Key", 0), "ANALOG", analogObj.GetValue("pAORGroup", 0), stlmrRecord.Record.ToString(), "N", analogObj.CurrentRecordNo.ToString(), 0, "4", "", "", stlmrRecord.Record, analogObj.GetValue("Name", 0), value };
            ...
            foreach (BescomFile tlmaFile in BescomParser.tlmaSet.Files.Values)
            {
                ...
                foreach (BescomRecord tlmaRecord in tlmaFile.ACSRecords)
                {
                    ...
                    int pRTU = GetpRTU(tlmaRecord, filenum);
                    accumulatorObj.SetValue("pRTU", pRTU);
                    ...
                    accumulatorObj.SetValue("Key", ScadaDB.GetNextKey(5, stationNumber));
                    SetAlarmGroups(analogObj, tlmaRecord);
                    ...
                    SetAlarmGroups(accumulatorObj, tlmaRecord);
                    object[] xrefObjects = { tlmaFile.Name, type, accumulatorObj.GetValue("pStation", 0), accumulatorObj.GetValue("Key", 0), "ACCUM", accumulatorObj.GetValue("pAORGroup", 0), tlmaRecord.Record.ToString(), "N", accumulatorObj.CurrentRecordNo.ToString(), accumulatorObj.GetValue("pRTU", 0), "0","0","0","0",accumulatorObj.GetValue("Name",0) };
            ...
            #region SubstationTLMA
            foreach (BescomFile stlmaFile in BescomParser.stlma.Files.Values)
            {
                int filenum = Convert.ToInt32(stlmaFile.Name.Split('.')[0].Replace("TLMA", ""));
                DataRow stationRow = BescomParser.Substation_names.GetInfo(new object[] { filenum });
                if (filenum == null)
                {
                    Logger.Log("No_RTU_Address_Match", LoggerLevel.INFO, $"No Match for file# found for RTU_Address in the master file for the input file : {stlmaFile.Name}"); //BD
                    continue;
                }

                foreach (BescomRecord stlmaRecord in stlmaFile.ACSRecords)
                {

                    if (stlmaRecord.InfoDict["point_description"] == "")
                    {
                        continue;
                    }
                    string name;
                    if (stlmaRecord.InfoDict["point_id"].Equals(""))
                    {
                        name = stlmaRecord.InfoDict["point_description"];
                    }
                    else
                    {
                        name = stlmaRecord.InfoDict["point_id"];
                    }
                    if (stlmaRecord.Record.Equals(0))
                    {
                        if (!String.IsNullOrEmpty(name)) Logger.Log("REC#0 Containing Information", LoggerLevel.INFO, $"Record#0 in TLMA contains information. Point Description: {name}");
                        continue;
                    }

                    int stationid = Convert.ToInt32(stlmaFile.Name.Split('.')[0].Replace("TLMA", ""));
                    if (stationid == 0) continue;
                    accumulatorObj.CurrentRecordNo++;

                    accumulatorObj.SetValue("Name", name);
                    
                    accumulatorObj.SetValue("pAORGroup", SetAor2(stlmaRecord, stationid));

                    accumulatorObj.SetValue("pStation", SetpStationSubstation(stlmaRecord, stationid));

                    SetAlarmGroups(analogObj, stlmaRecord);

                    SetAlarmGroups(accumulatorObj, stlmaRecord);
                    string type = "1";
                    accumulatorObj.SetValue("Type", type);

                    int stationNumber = Convert.ToInt32(stlmaFile.Name.Split('.')[0].Replace("TLMA", ""));
                    string maxcount = stlmaRecord.InfoDict["rollover_value"].ToString();
                    if (maxcount == "0.000000")
                    {
                        maxcount = "5.000000";
                    }
                    ...
                    accumulatorObj.SetValue("pScale", SetScale(ref ScadaDB, stlmaRecord));

                    SetAlarmGroups(accumulatorObj, stlmaRecord);
                    object[] xrefObjects = { "A", stlmaFile.Name, type, accumulatorObj.GetValue("pStation", 0), accumulatorObj.GetValue("Key", 0), "ANALOG", accumulatorObj.GetValue("pAORGroup", 0), stlmaRecord.Record.ToString(), "N", accumulatorObj.CurrentRecordNo.ToString(), accumulatorObj.GetValue("pRTU", 0), "0", accumulatorObj.GetValue("Name",0) };
            ...
            #region NONDASStation
            //foreach (BescomFile NDstspFile in BescomParser.NonDasstsp.Files.Values) //M_IND
            //{
            //    int filenum = Convert.ToInt32(NDstspFile.Name.Split('.')[0].Replace("STSP", ""));

            //    DataRow stationRow = BescomParser.Substation_names.GetInfo(new object[] { filenum });

            //    foreach (BescomRecord stspRecord in NDstspFile.ACSRecords)
            //    {
            //        string name = "";
            //        string base36 = string.Empty;
            //        string base36station = string.Empty;
            ...
            //            continue;
            //        }
            //        int stationNumber = Convert.ToInt32(NDstspFile.Name.Split('.')[0].Replace("STSP", ""));
            //        if (stationNumber == 0)
            //            continue;
            ...
            //        }
            //        statusObj.SetValue("State", 1);
            //        string middlenum = ScadaDB.DecToBase36(stationNumber, 3);

            //        int lastdigit = stspRecord.Record.ToInt();
            //        string lastnum = ScadaDB.DecToBase36(lastdigit, 3);
            //        const string chars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";
            //        base36 = chars[lastdigit % 36] + base36;
            //        lastdigit /= 36;
            //        if (base36.Length == 1) newnum = base36.PadLeft(3, '0');
            //        if (base36.Length == 2) newnum = base36.PadLeft(1, '0');
            //        base36station = chars[filenum % 36] + base36station;
            //        filenum /= 36;
            //        string key = "12" + middlenum + lastnum;
            //        int stationid = Convert.ToInt32(NDstspFile.Name.Split('.')[0].Replace("STSP", ""));

            //        statusObj.SetValue("pAORGroup", SetAor2(stspRecord, stationid));

            //        statusObj.SetValue("pStation", SetpStationSubstation(stspRecord, stationid));
            ...
            //        string typec = statusObj.GetValue("Type", 0).ToString();
            //        string feptype = typec == "2" ? "1" : typec == "1" ? "1" : "0";
            //        object[] xrefObjects = { NDstspFile.Name, type, statusObj.GetValue("pStation", 0), key, "STATUS", statusObj.GetValue("pAORGroup", 0), stspRecord.Record.ToString(), "P", statusObj.CurrentRecordNo.ToString(), statusObj.GetValue("pRTU", 0), feptype, statusObj.GetValue("Name", 0), value, pState };
            //        { "filename", "Type", "OSI Station", "SCADA Key", "Object", "OSI AOR", "Point Address", "Calculated", "OSI Record" };//stsp
            //        Xref.AddRecordSetValues(xrefObjects);
            //    }
            //}
            ...
            XrefSUB.WriteToDefault("BESCOMSUB_XREF");
        ...
        public static int SetpStationSubstation(BescomRecord record, int stationid)
        {
            ...
            if (stationRow != null)
            {
                int.TryParse(stationRow["pStation"].ToString(), out int stationNum);
                return stationNum;
            }
            else
                return 1;
        ...
        public static void SetAnalogLimits(DbObject analogObj, BescomRecord acsRecord)
        {
            ...
            double high0, high1, high2, high3, high4 = 0.00;
            double low0, low1, low2, low3, low4 = 0.00;
            high0 = 0; high1=0; low0 = 0; low1= 0;
            high2 = Convert.ToDouble(acsRecord.InfoDict["high_operational_limit"]);
            high3 = Convert.ToDouble(acsRecord.InfoDict["high_emergency_limit"]);
            high4 = Convert.ToDouble(acsRecord.InfoDict["high_reasonability_limit"]);
            low2 = Convert.ToDouble(acsRecord.InfoDict["low_operational_limit"]);
            low3 = Convert.ToDouble(acsRecord.InfoDict["low_emergency_limit"]);
            low4 = Convert.ToDouble(acsRecord.InfoDict["low_reasonability_limit"]);
            if (low2 == 8.0)
            {
                low2 = -8.0;
            }
            analogObj.SetValue("NominalHiLimits", 0, high0);
            analogObj.SetValue("NominalHiLimits", 1, high1);
            analogObj.SetValue("NominalHiLimits", 2, high2);
            analogObj.SetValue("NominalHiLimits", 3, high3);
            analogObj.SetValue("NominalHiLimits", 4, high4);
            analogObj.SetValue("NominalLowLimits", 0, low0);
            analogObj.SetValue("NominalLowLimits", 1, low1);
            analogObj.SetValue("NominalLowLimits", 2, low2);
            analogObj.SetValue("NominalLowLimits", 3, low3);
            analogObj.SetValue("NominalLowLimits", 4, low4);
           ...
           if(high0==0 && low0==0 && high1==0 && low1==0 )
            {
                nomPairInactive[1] = 1;
                nomPairInactive[2] = 1;
                nomPairInactive[3] = 0;
            ...
            if (high0 == 0 && low0 == 0 && high1 == 0 && low1 == 0 && high2==0 && low2==0 && high3==0 && low3==0)
            {
                nomPairInactive[0] = 0;
                nomPairInactive[1] = 1;
                nomPairInactive[2] = 1;
            ...
            if (high0 == 0 && low0 == 0 && high1 == 0 && low1 == 0 && high2 == 0 && low2 == 0 && high3 == 0 && low3 == 0 && high4==0 && low4==0)
            {
                nomPairInactive = new int[] { 1, 1, 1, 1, 1, 1, 1, 1 };
                


            }
            if (high0 == 0 && low0 == 0 && high1 == 0 && low1 == 0 && high2 == 0.01 && low2 == 0 && high3 == 0 && low3 == 0 && high4 == 0 && low4 == 0)
            {
                nomPairInactive = new int[] { 1, 1, 1, 1, 1, 1, 1, 1 };

            }
            if (high0 == 0 && low0 == 0 && high1 == 0 && low1 == 0 && (high2 == 11.7|| high2==11.66|| high2==11.6) && (low2 == 0|| low2 == 10.3||low2==10.34) && high3 == 0 && low3 == 0 && high4 == 12.2 && low4 == 9.5)
            {
                nomPairInactive = new int[] { 0, 1, 1, 1, 0, 1, 1, 0 };
                if((high2==11.7||high2==11.66) && low2==0 && low4==9.5)
                {
                    low2 = -9.5;
                    analogObj.SetValue("NominalLowLimits", 2, low2);
                }
            ...
            if (high0 == 0 && low0 == 0 && high1 == 0 && low1 == 0 && (high2 == 11.7 || high2 == 11.66 || high2 == 11.6) && (low2 == 0 || low2 == 10.3 || low2 == 10.34) && high3 == 0 && low3 == 0 && high4 == 12.2 && low4 == 9.5)
            {
                nomPairInactive = new int[] { 0, 1, 1, 0, 1, 1, 1, 0 };
                if ((high2 == 11.7 || high2 == 11.6 || high2 == 11.66) && low2 == 0 && low3 == 9.5)
                {
                    low2 = -9.5;
                    analogObj.SetValue("NominalLowLimits", 2, low2);
                }
            }
            if (high0 == 0 && low0 == 0 && high1 == 0 && low1 == 0 && (high2 == 11.7 || high2 == 0 || high2 == 11.66) && low2 == 0 && high3 == 12 && low3 == 10 && high4 == 12.2 && low4 == 9.5)
            {
                nomPairInactive = new int[] { 0, 1, 1, 0, 1, 1, 1, 0 };

            }

            if (high0 == 0 && low0 == 0 && high1 == 0 && low1 == 0 &&  high2 == 0  && low2 == 0 && high3 == 12 && low3 == 10 && high4 == 12.2 && low4 == 9.5)
            {
                nomPairInactive = new int[] { 0, 1, 1, 0, 1, 1, 1, 0 };
